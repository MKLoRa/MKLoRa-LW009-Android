...
public class DeviceSettingActivity extends Lw009BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mBind.layoutAdvanceSet.setOnClickListener(v -> {
            if (isTriggerValid()) {
                    ...
                    switch (keyEnum) {
                        ...
                        case KEY_SET_HEART_INTERVAL:
                            if (length == 1) {
                                int result = value[3] & 0xff;
                                if (result == 0) {
                                    MokoSupport.getInstance().sendOrder(OrderTaskAssembler.setDetectionAlgorithms(selectedDetectionAlgorithms));
                                } else {
                                    dismissSyncProgressDialog();
                                    ToastUtils.showToast(this, "set up failed");
                                }
                        ...
                        case KEY_SET_DETECTION_ALGORITHMS:
                            if (length == 1) {
                                int result = value[3] & 0xff;
                                if (result == 0) {
                                    MokoSupport.getInstance().sendOrder(OrderTaskAssembler.setDetectionSensitivity(selectedSensitivity));
                                } else {
                                    dismissSyncProgressDialog();
                                    ToastUtils.showToast(this, "set up failed");
                                }
